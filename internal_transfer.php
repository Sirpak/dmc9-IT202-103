...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($source_account === $destination_account) {
        $result = flashMessage("Source and destination accounts must be different.");
    } elseif ($amount <= 0) {
        ...
        try {
            ...
            // Validate ownership and balances
            ...
            $stmt = $db->prepare("SELECT id, balance FROM accounts WHERE id = :id AND user_id = :user_id");
            $stmt->execute([':id' => $destination_account, ':user_id' => $_SESSION['id']]);
            ...
            $expected_destination_balance = $destination['balance'] + $amount;

            $stmt = $db->prepare("INSERT INTO transactions (account_src, account_dest, balance_change, transaction_type, memo, expected_total) 
                                  VALUES 
                                  (:src, :dest, -:amount, 'transfer', :memo, :expected_src_total),
                                  (:src, :dest, :amount, 'transfer', :memo, :expected_dest_total)");
            $stmt->execute([
                ':src' => $source_account,
                ...
                ':expected_dest_total' => $expected_destination_balance,
            ]);
            ...
            $stmt->execute([':balance' => $expected_destination_balance, ':id' => $destination_account]);

            $db->commit();
            $result = flashMessage("Internal transfer successful!", "Pass");
        } catch (Exception $e) {
            $db->rollBack();
...
<div class="container mt-5">
    <h1 class="text-center">Internal Transfer</h1>
    ...
    <form action="" method="POST" class="mt-4">
        ...
        <div class="mb-3">
            <label for="destination_account" class="form-label">Destination Account</label>
            <select name="destination_account" id="destination_account" class="form-select" required>
